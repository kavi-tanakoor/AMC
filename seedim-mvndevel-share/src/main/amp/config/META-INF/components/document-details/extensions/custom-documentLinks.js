...
(function()
{
   ...
   YAHOO.extend(Alfresco.customDocumentLinks, Alfresco.component.Base,
   {
       ...
       * @method: onReady
       */
      onReady: function customDocumentLinks_onReady()
      {
         // Display copy links
         ...
         //Get Document Node
         var n = this.options.documentNode;
         var docProperties = ((n.item).node).properties;
         var docID = "Not Available";
         for (var prop in docProperties){
        	 if(prop == "cm:name"){
        		 docID = docProperties[prop];
        	 }
         }
         var url = window.location.protocol + "//" + window.location.host + "/share/page/service/details/" + docID;
         // Prefix some of the urls with values from the client
         Dom.get(this.id + "-page").value = url;
      },
